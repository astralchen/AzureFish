...
AzureFish 负责客户端界面、交互、网络调用与本地数据；AzureFishServer 是独立 Swift 服务端，负责用户身份、权威资料及后续 IM。两端数据库不共享文件。网络采用 HTTP＋Apple SwiftProtobuf，未来实时 IM 使用 WebSocket；传输安全遵循下文。服务端是网络契约的唯一来源，客户端不另行维护一套 `.proto`；生成的客户端类型应纳入本工程，确保无需相邻服务端目录也能独立构建。
