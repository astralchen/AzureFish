...
func registration(_ name: String = "test_account") -> Azurefish_V1_RegisterRequest {
...
func auth(_ app: Application, name: String = "test_account") async throws -> Azurefish_V1_AuthResponse {
    let response = try await send(app, .POST, "/v1/auth/register", registration(name))
    #expect(response.status == .created)
    return try decode(Azurefish_V1_AuthResponse.self, response)
}
func refresh(_ token: String) -> Azurefish_V1_RefreshRequest {
    var request = Azurefish_V1_RefreshRequest(); request.operationID = UUID().uuidString; request.refreshToken = token
    return request
}
...
func errorCode(_ response: TestingHTTPResponse) throws -> String { try decode(Azurefish_V1_ApiError.self, response).code }
...
struct AccountTests {
    @Test func registerLoginProfileLogout() async throws {
        try await withServer { (app: Application, _: Fixture) async throws in
            ...
            let first = try decode(Azurefish_V1_AuthResponse.self, result)
            ...
            var login = Azurefish_V1_LoginRequest()
            ...
            let loggedIn = try decode(Azurefish_V1_AuthResponse.self, await send(app, .POST, "/v1/auth/login", login))
            #expect(loggedIn.userID == first.userID)
            #expect(loggedIn.sessionID != first.sessionID)
            let loginAgain = try decode(Azurefish_V1_AuthResponse.self, await send(app, .POST, "/v1/auth/login", login))
            #expect(loginAgain.accessToken == loggedIn.accessToken)
            #expect(try await me(app, loggedIn.accessToken).status == .ok)
            var logout = Azurefish_V1_LogoutRequest(); logout.operationID = UUID().uuidString
    ...
    @Test func refreshRecoverySupersessionAndReplayRevocation() async throws {
        try await withServer { (app: Application, _: Fixture) async throws in
            ...
            let second = try decode(Azurefish_V1_AuthResponse.self, result)
            ...
            let third = try decode(Azurefish_V1_AuthResponse.self, thirdResult)
    ...
    @Test func profileRetrySurvivesTokenRotation() async throws {
        try await withServer { (app: Application, _: Fixture) async throws in
            let first = try await auth(app)
            var patch = Azurefish_V1_UpdateProfileRequest()
            ...
            let renewed = try decode(Azurefish_V1_AuthResponse.self, await send(app, .POST, "/v1/auth/refresh", refresh(first.refreshToken)))
            ...
            #expect(try decode(Azurefish_V1_UserProfile.self, await me(app, renewed.accessToken)).profileVersion == 2)
        }
    }
    ...
    @Test func expiredRefreshRecoveryNeverExecutesAgain() async throws {
        try await withServer { (app: Application, fixture: Fixture) async throws in
            let first = try await auth(app)
            let request = refresh(first.refreshToken)
            let second = try decode(Azurefish_V1_AuthResponse.self, await send(app, .POST, "/v1/auth/refresh", request))
    ...
    @Test func profilePresenceConflictsAndIsolation() async throws {
        try await withServer { (app: Application, _: Fixture) async throws in
            ...
            let profile = try decode(Azurefish_V1_UserProfile.self, saved)
            ...
            #expect(try decode(Azurefish_V1_UserProfile.self, await send(app, .PATCH, "/v1/me", patch, token: a.accessToken)).bio == "")
            let other = try decode(Azurefish_V1_UserProfile.self, await me(app, b.accessToken))
    ...
    @Test func expiryAndRecoveryWindow() async throws {
        try await withServer { (app: Application, fixture: Fixture) async throws in
            ...
            let result = try decode(Azurefish_V1_AuthResponse.self, await send(app, .POST, "/v1/auth/register", request))
            fixture.clock.advance(601)
            ...
            let renewed = try decode(Azurefish_V1_AuthResponse.self, await send(app, .POST, "/v1/auth/refresh", refresh(result.refreshToken)))
    ...
    @Test func restartPreservesSessionAndRefreshRecovery() async throws {
        ...
        let session: Azurefish_V1_AuthResponse
        let request: Azurefish_V1_RefreshRequest
        let refreshed: Azurefish_V1_AuthResponse
        do {
            session = try await auth(first)
            request = refresh(session.refreshToken)
            refreshed = try decode(Azurefish_V1_AuthResponse.self, await send(first, .POST, "/v1/auth/refresh", request))
        ...
        do {
            ...
            let recovered = try decode(Azurefish_V1_AuthResponse.self, await send(second, .POST, "/v1/auth/refresh", request))
